...
function wpstarter_entry_footer() {
	...
	if ( 'post' === get_post_type() ) {
		...
		if ( $categories_list && wpstarter_categorized_blog() ) {
			printf( '<span class="cat-links"><i class="fa fa-folder-open"></i>' . esc_html__( 'Posted in %1$s', 'wpstarter' ) . '</span>', $categories_list ); // WPCS: XSS OK.
		}

		/* translators: used between list items, there is a space after the comma */
		$tags_list = get_the_tag_list( '', esc_html__( ', ', 'wpstarter' ) );
		if ( $tags_list ) {
			printf( '<span class="tags-links"><i class="fa fa-tags"></i>' . esc_html__( 'Tagged %1$s', 'wpstarter' ) . '</span>', $tags_list ); // WPCS: XSS OK.
		}
	}
